...
// Blocklist helper (block motorsnag.com and specific bad URLs/repos)
function isBlockedUrl(url) {
  try {
    if (!url) return false;
    const s = String(url).trim().replace(/^['"]|['"]$/g, '').toLowerCase();
    // Exact blocked URLs
    const blockedExact = [
      'https://motorsnag.com/qph9bfad?key=17276614cc8dc8ab4e18cf78200ebeb4'
    ];
    ...
    return host.includes('motorsnag.com') || s.includes('gn-math-dontdmca');
  } catch (e) { return false; }
}
// Prevent user clicks from opening blocked domains (motorsnag.com)
document.addEventListener('click', function (e) {
  try {
    var a = e.target.closest && e.target.closest('a');
    if (a && a.href && isBlockedUrl(a.href)) {
      e.preventDefault();
      alert('Opening that external site is blocked.');
    }
  } catch (err) {}
});

// Try to inject HTML into a compiler/preview iframe if present
function tryInjectIntoCompiler(html, meta) {
  const ids = ['compilerFrame','compiler','editorFrame','editor','playgroundFrame','playground','previewFrame','preview','outputFrame','output'];
  for (const id of ids) {
    try {
      const el = document.getElementById(id);
      if (!el) continue;
      if (el.tagName && el.tagName.toLowerCase() === 'iframe') {
        if ('srcdoc' in el && html !== null) {
          el.srcdoc = html;
        } else if (el.contentDocument && html !== null) {
          el.contentDocument.open();
          el.contentDocument.write(html);
          el.contentDocument.close();
        } else if ('srcdoc' in el && html === null && meta && meta.url) {
          el.src = meta.url;
        } else if (html === null && meta && meta.url) {
          el.src = meta.url;
        } else {
          continue;
        }
        try { if (meta && meta.name) document.getElementById('zoneName').textContent = meta.name; } catch(e){}
        try { if (meta && meta.id) document.getElementById('zoneId').textContent = meta.id; } catch(e){}
        try { if (meta && meta.author) document.getElementById('zoneAuthor').textContent = 'by ' + meta.author; } catch(e){}
        return true;
      }
            ...
            if (zone) {
                if (embed) {
                    if (zone.url.startsWith("http")) {
                      if (isBlockedUrl(zone.url)) {
                        alert('Opening that site is blocked.');
                      } else {
...
function openZone(file) {
    if (file.url.startsWith("http")) {
      if (isBlockedUrl(file.url)) {
        alert('Opening that site is blocked.');
        return;
      }
      // Try in-page compiler/preview first
      try {
        if (tryInjectIntoCompiler(null, {url: file.url, name: file.name, id: file.id, author: file.author})) return;
      } catch(e){}
      window.open(file.url, "_blank");
    } else {
        const url = file.url.replace("{COVER_URL}", coverURL).replace("{HTML_URL}", htmlURL);
        fetch(url+"?t="+Date.now()).then(response => response.text()).then(html => {
          // Try injecting into compiler/preview iframe first
          try {
            if (tryInjectIntoCompiler(html, {name: file.name, id: file.id, author: file.author})) return;
          } catch(e) {}
...
function aboutBlank() {
  ...
  if (isBlockedUrl(zone)) { alert('Opening that site is blocked.'); if (newWindow) newWindow.close(); return; }
  fetch(zone+"?t="+Date.now()).then(response => response.text()).then(html => {
    // Try injecting into compiler/preview iframe first
    try {
      if (tryInjectIntoCompiler(html)) {
